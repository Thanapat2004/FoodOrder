...
export default function Addtocart({ paymentMethods }) {
    ...
    const [userAddress, setUserAddress] = useState({ name: '', phone: '', address: '' });

    useEffect(() => {
        const storedCart = JSON.parse(localStorage.getItem('cart')) || [];
        const storedAddress = JSON.parse(localStorage.getItem('userAddress')) || { name: '', phone: '', address: '' };
        ...
        if (loggedInUser && currentUser !== loggedInUser) {
            ...
            setCart([]);
            setUserAddress({ name: '', phone: '', address: '' });
        }
    }, []);
    ...
    const handleSubmitOrder = () => {
        ...
        Inertia.post(route('orders.store'), orderData, {
            ...
            onSuccess: () => {
                ...
                localStorage.removeItem('cart');
                localStorage.removeItem('userAddress'); // ลบข้อมูลที่อยู่ออกจาก localStorage ด้วย
                setCart([]);
                setSelectedPayment('');
                setUserAddress({ // เคลียร์ข้อมูลในฟอร์มที่อยู่
                    name: '',
                    phone: '',
                    address: ''
                });
