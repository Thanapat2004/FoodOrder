...
export default function Index({ foods = [], categories = [] }) {
    ...
    <h1 className="text-3xl font-bold text-gray-800">ระบบการจัดการอาหาร</h1>
             ...
             จัดการคำสั่งซื้อ
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-6">
                    {filteredFoods.map((food) => (
                        <div key={food.id} className="border rounded-lg p-4 shadow-lg hover:shadow-xl transition-all bg-white overflow-hidden">
                            {food.image_url && (
                                <img 
                                    src={food.image_url} 
                                    alt={food.name} 
                            ...
                            <div className="p-3">
                                <h2 className="text-xl font-semibold mb-1 text-gray-900">{food.name}</h2>
                                <p className="text-gray-600 mb-1 line-clamp-2">{food.description}</p>
                                <p className="text-gray-500 mb-1">หมวดหมู่: {food.category?.name || 'ไม่มีหมวดหมู่'}</p>
